...
const TABS: { id: Tab; label: string }[] = [
  { id: "verified", label: "LIVE CONFIGS" },
...
function SkeletonCard() {
  return (
    <div className="animate-pulse rounded-sm border border-border bg-card p-5">
      <div className="mb-4 h-32 rounded-sm border border-border bg-secondary/70" />
      <div className="h-4 w-24 rounded-sm bg-secondary/70" />
      <div className="mt-3 h-3 w-full rounded-sm bg-secondary/60" />
      <div className="mt-2 h-3 w-3/4 rounded-sm bg-secondary/60" />
      <div className="mt-4 flex gap-2">
        <div className="h-8 flex-1 rounded-sm bg-secondary/70" />
        <div className="h-8 w-10 rounded-sm bg-secondary/70" />
      </div>
    </div>
  )
}
...
export function CfgHub({ initialCommunityConfigs }: { initialCommunityConfigs: CfgConfig[] }) {
  ...
  const { data: communityConfigs = [], mutate, isLoading } = useSWR<CfgConfig[]>("community-configs", getCommunityConfigs, {
  ...
  if (!hydrated) {
    return (
      <section className="mx-auto w-full max-w-5xl px-4 pb-28">
        <div className="mb-6 h-16 rounded-sm border border-border bg-card/70" />
        <div className="mb-6 grid gap-2 sm:grid-cols-2">
          <div className="h-10 rounded-sm border border-border bg-card/70" />
          <div className="h-10 rounded-sm border border-border bg-card/70" />
        </div>
        <div className="mb-8 h-12 rounded-sm border border-border bg-card/70" />
  ...
  const loading = Boolean(isLoading && (communityConfigs.length === 0))
    ...
    <section className="mx-auto w-full max-w-5xl px-4 pb-28">
      ...
      >
        {TABS.map((t) => (
          <button
            ...
            className={`relative -mb-px flex items-center gap-2 px-1 py-3 text-left font-mono text-xs uppercase tracking-widest transition-all duration-300 sm:px-3 ${
      ...
      <div className="relative mb-8">
        ...
        <input
          ...
          className="cfg-input pl-9 transition-all duration-300 focus:border-primary"
        ...
        <>
          {loading ? (
            ...
            <p className="py-16 text-center font-mono text-sm text-muted-foreground">
              {query ? `no live configs match "${query}"` : "no live configs are available yet."}
            </p>
          ) : (
            <div className={`grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 transition-opacity duration-300 ${
              loading ? "opacity-60" : "opacity-100"
            }`}>
              {verifiedResults.map((config, i) => (
                <ConfigCard key={config.id} config={config} index={i} verified />
              ))}
            </div>
          )}
        </>
      ...
      {tab === "community" && !searching && activeFolder && (
        <>
          ...
          <p className="fixed bottom-6 right-6 z-40 rounded-sm border border-border bg-popover/95 px-4 py-2.5 font-mono text-xs text-muted-foreground shadow-lg backdrop-blur-md">
            <span className="text-foreground">Author(s):</span> {activeFolder.contributors.join(", ")}
          </p>
        </>
      )}
    </section>
